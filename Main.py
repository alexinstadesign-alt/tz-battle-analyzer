...
<body>
    ...
    <script>
        ...
        // Функции сортировки
        function sortTable(field, isNumeric) {{
        ...
        function resetSort() {{
            currentSort = {{ field: 'Total', direction: 'desc' }};
            updateTable();
        }}
        ...
        function applyFilters() {{
            ...
            Object.entries(allPlayersData).forEach(([player, data]) => {{
                const filteredBattles = data.battle_details.filter(battle => {{
                    const battleDate = new Date(battle.time);
                    return battleDate >= startDate && battleDate <= endDate;
                }});
                
                if (filteredBattles.length > 0) {{
                    const filteredData = {{
                        battles_count: filteredBattles.length,
                        Metals: 0,
                        'Precious metals': 0,
                        Polymers: 0,
                        Organic: 0,
                        ...
                        Venom: 0,
                        Total: 0,
                        battle_details: filteredBattles
                    }};
                    
                    filteredBattles.forEach(battle => {{
                        Object.entries(battle.resources).forEach(([resource, amount]) => {{
                            if (filteredData.hasOwnProperty(resource)) {{
                                filteredData[resource] += amount;
                            }}
                        }});
                    }});
                    
                    filteredData.Total = filteredData.Metals + filteredData['Precious metals'] + 
                                       filteredData.Polymers + filteredData.Organic + 
                                       filteredData.Silicon + filteredData.Radioactive + 
                                       filteredData.Gems + filteredData.Venom;
                    
                    currentPlayersData[player] = filteredData;
                }}
            }});
        ...
        function resetFilters() {{
            document.getElementById('startDate').value = '{min_date.strftime('%Y-%m-%dT%H:%M')}';
            document.getElementById('endDate').value = '{max_date.strftime('%Y-%m-%dT%H:%M')}';
            currentPlayersData = {{...allPlayersData}};
            updateTable();
        }}
        ...
        function showPlayerDetails(player) {{
            ...
            modalTitle.textContent = `Детали боев игрока ${{player}}`;
            playerDetails.innerHTML = '';
            
            const playerData = allPlayersData[player];
            if (playerData && playerData.battle_details) {{
                playerData.battle_details.sort((a, b) => new Date(b.time) - new Date(a.time));
                
                playerData.battle_details.forEach(battle => {{
                    const battleDiv = document.createElement('div');
                    battleDiv.className = 'battle-item';
                        ...
                        if (amount > 0) {{
                            resourcesHTML += `
                                <div class="resource-item">
                                    <img src="${{resourceIcons[resource]}}" class="resource-icon" alt="${{resource}}">
                                    <span>${{resource}}: ${{amount.toLocaleString()}}</span>
                                </div>
                            `;
                        }}
                    }});
                    ...
                    battleDiv.innerHTML = `
                        <div class="battle-header">
                            Бой #${{battle.battle_id}} - ${{new Date(battle.time).toLocaleString()}}
                        </div>
                        <div>Локация: ${{battle.location}}</div>
                        <div>Всего ресурсов: ${{battle.total_resources.toLocaleString()}}</div>
                        <div class="battle-resources">
                            ${{resourcesHTML}}
                        </div>
                    `;
                    playerDetails.appendChild(battleDiv);
                }});
            }}
        ...
        // Экспорт данных
        function exportData() {{
            const dataStr = JSON.stringify(currentPlayersData, null, 2);
            const dataBlob = new Blob([dataStr], {{type: 'application/json'}});
            const url = URL.createObjectURL(dataBlob);
            const link = document.createElement('a');
            link.href = url;
            link.download = 'battle_data.json';
            link.click();
